...
function Masterhead() {
  ...
  const loadingSkeleton = Array.from([1, 2]).map((i) => (
    <div key={i} className='animate-pulse rounded-lg h-2 w-full bg-[var(--background-color)]'></div>
  ));
    ...
    <div className='flex flex-col gap-3'>
      <section className='space-y-2'>
        ...
        <ul className={`${loading && 'space-y-6'}`}>
          {loading
            ? loadingSkeleton
            : editors.map((editor, i) => <li key={editor.name + i}>{editor.name}</li>)}
        </ul>
      </section>
      <section className='space-y-2'>
        ...
        <ul className={`${loading && 'space-y-6'}`}>
          {loading
            ? loadingSkeleton
            : managingEditors.map((editor, i) => <li key={editor.name + i}>{editor.name}</li>)}
        </ul>
      </section>
      <section className='space-y-2'>
        ...
        <ul className={`${loading && 'space-y-6'}`}>
          {loading
            ? loadingSkeleton
            : consultingEditors.map((editor, i) => <li key={editor.name + i}>{editor.name}</li>)}
        </ul>
      </section>
    </div>
  );
